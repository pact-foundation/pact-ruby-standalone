...
TRAVELING_RUBY_VERSION = "#{TRAVELING_RUBY_VERSION_REL_DATE}-3.1.4"
...
namespace :package do
  ...
  task :bundle_install do
    if RUBY_VERSION !~ /^3\.1\./
      abort "You can only 'bundle install' using Ruby 3.1.4, because that's what Traveling Ruby uses."
    end
    sh "rm -rf build/tmp"
